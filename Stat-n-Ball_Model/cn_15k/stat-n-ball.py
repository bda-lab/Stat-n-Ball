...
Original file is located at
    https://colab.research.google.com/drive/1_JTy4B3ZEOxa3xR8vjJB6ibBvaYhqSUB
"""
...
import pandas as pd
cn_train_sc=pd.read_csv(df_train_file)

sbc=cn_train_sc[cn_train_sc['relation'].isin(['isa','mannerof'])]
sbc.head()
...
df_filt=df[df['Entity2'].isin(list_ent3)]
df_filt_s=df_filt.sort_values(['Entity2','Prob_Score'],ascending=False).groupby('Entity2').head(100000)
...
from tensorflow import constraints
from tensorflow.callbacks import ModelCheckpoint, EarlyStopping, CSVLogger
...
#gdata_file="cn_15k_prob_jul8/axiom_train_all2prob_31may.owl"
...
def ndcg_(a,b):
    ...
    #return tf.math.exp(tf.reduce_sum(add,axis=1)[1]/tf.reduce_sum(add,axis=1)[0])
...
class ELModel(tf.keras.Model):
    ...
    def call(self, input):
        """Run the model."""
        nf1, nf2, nf3, dis, top, nf3_neg,radius, sub_info = input
        ...
        #loss=loss8
        loss = loss1 + loss2+loss3+ loss_dis + loss_top + loss_nf3_neg-loss8 - loss7
        #loss=loss8
        return loss
    ...
    def nf1_neg_loss(self, input):
        ...
        tot=tf.where(tf.math.greater(cr,dr),tot,tot*reduction_factor)
        timea=tf.clip_by_value(tot, clip_value_min=1,clip_value_max=mu_high)###
        #timea=rsum-timea
    ...
    def nf2_loss(self, input):
        ...
        dst_loss = (tf.nn.relu(dst - sr - self.margin)
                    ...
                    + rdst)
        dst_loss=tf.reshape(dst_loss,[-1])
        rdiff=rda-rca
        rsum=rda+rca
        mu_high=tf.math.divide_no_nan(rsum, rdiff)
        hlf=((tf.cast(f,tf.float32)-1.0)*(mu_high-1.0))/(0.0-1.0)
        tot=1.0+hlf
        reduction_factor=  tf.math.divide_no_nan(rca, rda)
        tot=tf.where(tf.math.greater(rda,rca),tot,tot*reduction_factor)
        timea=tf.clip_by_value(tot, clip_value_min=1,clip_value_max=mu_high)###
        timea=tf.reshape(timea,[-1])
        #timea=rsum-timea
        #dst_loss2=tf.where(tf.equal(dst_loss,0),timea,timea*dst_loss)
        dst=tf.where(tf.equal(dst_loss,0.0),tf.where(tf.equal(timea,0.0),0.0,tf.nn.relu(timea)),tf.nn.relu(timea*(dst_loss)))
        #return timea*dst_loss + self.reg(x1) + self.reg(x2) + self.reg(x3)
        dst=tf.reshape(dst,[-1,1])
        return dst + self.reg(x1) + self.reg(x2) + self.reg(x3)
    ...
    def nf2_neg_loss(self, input):
        ...
        dst_loss2=tf.where(tf.equal(dst_loss,0),timea,timea*dst_loss)
        #return timea*dst_loss + self.reg(x1) + self.reg(x2) + self.reg(x3)
        dst_loss2=tf.where(tf.equal(euc + rc - rd - margin,0.0),tf.where(tf.equal(timea,0.0),0.0,tf.nn.relu(timea)),tf.nn.relu(timea*(euc + rc - rd - margin)))
    ...
    def dis_loss(self, input):
        ...
        dst=tf.where(tf.equal(sr - euc + self.margin,0.0),tf.where(tf.equal(timea,0.0),0.0,tf.nn.relu(timea)),tf.nn.relu(timea*(sr - euc + self.margin)))
        dst=tf.reshape(dst,[-1,1])
        return dst + self.reg(x1) + self.reg(x2)
    ...
    def sub_info_loss(self,input):
        ...
        abc=[]
        #tf.where(tf.less(tf.reduce_sum(a),tf.reduce_sum(b)),ndcg_(b,a),hdm_(b,a))
        for iiv in ind_ndcg:
               b=rnk_flg_tf3[iiv]
               a=rank_full[iiv]
        ...
        abc2=[]
        for iiv in ind_ndcg:
               abc2.append(hdm_(rnk_flg_tf3[iiv],rank_full[iiv]))"""
        azi=tf.convert_to_tensor(abc)
        #abc2=tf.convert_to_tensor(abc2)
        #azi=azi-abc2
...
class Generator(object):
    ...
    def next(self):
        if self.start < self.steps:
            ...
            self.start += 1
            return ([nf1, nf2, nf3, dis, top, nf3_neg,radius,sub_info], labels)
        else:
            self.reset()
...
def build_model(device,train_data,classes,relations,valid_data):
    ...
    with tf.device('/' + device):
        ...
        out = el_model([nf1,nf2, nf3,  dis, top, nf3_neg,radius,sub_info])
        model = tf.keras.Model(inputs=[nf1, nf2, nf3, dis, top, nf3_neg,radius,sub_info], outputs=out)
...
for d in embed_dims:
    ...
    for m in margins:
        ...
        out_classes_file = f'cn_15k_results/EmEL_dir/GO_{embedding_size}_{margin}_{epochs}_{save_file_name}_cls.pkl'
        out_relations_file = f'cn_15k_results/EmEL_dir/GO_{embedding_size}_{margin}_{epochs}_{save_file_name}_rel.pkl'
        loss_history_file= f'cn_15k_results/EmEL_dir/GO_lossHis_{embedding_size}_{margin}_{epochs}.csv'
